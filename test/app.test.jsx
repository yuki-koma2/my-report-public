...
describe("App", () => {
  it("ホームにレポート一覧を表示する", () => {
    ...
    expect(screen.getByRole("heading", { name: "医療・介護領域 週次最新情報サンプル" })).toBeInTheDocument();
    expect(screen.getByRole("heading", { name: "医療DX標準化の深掘り調査サンプル" })).toBeInTheDocument();
    expect(screen.getByText("2件のサンプル記事を掲載中")).toBeInTheDocument();
  });
  ...
  it("レポート詳細に公開日、確認日、出典を表示する", () => {
    ...
    expect(screen.getByRole("heading", { name: "医療・介護領域 週次最新情報サンプル", level: 1 })).toBeInTheDocument();
    ...
    expect(screen.getByRole("link", { name: "厚生労働省 公開資料" })).toHaveAttribute("href", "https://www.mhlw.go.jp/");
  });
  ...
  it("タグページに該当記事を表示する", () => {
    ...
    expect(screen.getByRole("heading", { name: "医療DX標準化の深掘り調査サンプル" })).toBeInTheDocument();
    expect(screen.queryByRole("heading", { name: "医療・介護領域 週次最新情報サンプル" })).not.toBeInTheDocument();
  });
